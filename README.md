...
以 HTTP `202` 返回；Word、PDF、TXT 等解析、切片和检索索引通过 FastAPI `BackgroundTasks`
在响应发送后执行，不再要求单独启动 `python -m app.ingestion_worker`。API 进程必须由 Supervisor
常驻；如果 API 进程在后台任务完成前退出，该任务不会跨进程恢复。Excel/CSV 完成表结构预览后，
后续正式结构化发布仍由 `python -m app.structured_worker` 执行，该 Worker 仍需由 Supervisor 常驻。
