...
def _publish_secret_set(paths: Mapping[str, Path]) -> None:
    ...
    try:
        ...
        for name in reversed(backed_up):
            _replace_secret(backup_dir / name, paths[name])
        raise
    finally:
...
def _validate_secret_set(paths: Mapping[str, Path]) -> None:
    ...
    for password in (postgres_password, query_password, ingest_password):
...
def prepare_environment(
    ...
    *,
    ...
    _assert_regular_non_link(env_path, "Offline environment")
    ...
    if created_env:
        set_env_value(env_path, "DCAGENT_UID", uid)
        set_env_value(env_path, "DCAGENT_GID", gid)
    ...
    if present and present != set(paths):
        raise DeploymentError("Managed offline secrets must exist as one complete set")
    if rotate_secrets or not present:
        _publish_secret_set(paths)
